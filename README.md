...
```
docker volume create --driver local --opt type=none --opt device=/full-stack-apache2-drupal-for-everyone-with-docker-compose/certbot --opt o=bind certbot-etc
```
...
00 01 * * * tar -czvf /${DIRECTORY_PATH}/backups/'backup-example.com-'$(date +"\%Y-\%m-\%dT\%H-\%M-\%S")'.tar.gz' --exclude='drupal/core' --exclude='backups' /${DIRECTORY_PATH}
